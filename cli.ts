...
import { startAgentViewer } from './core/viewer.ts';
...
async function createAgentWithAI(
  ...
  if ((await exists(agentDir)) && (await exists(agentJsonPath))) {
    try {
      ...
      console.log(
        colorize(`   Previous attempts: ${existingHistory.attempts?.length || 0}`, 'gray')
      );
  ...
  console.log(
    colorize(
      `\n${emoji} ${isContinuation ? 'Continuing' : 'Creating'} ${entityType}: ${colorize(
        name,
        'bold'
      )}`,
      themeColor
...
`;
    ...
    console.log(
      colorize(
        `\n   ✨ AI-generated ${entityType} created at: ${colorize(
          prepResult.files.indexFile,
          'bold'
        )}`,
        'green'
      )
    ...
    if (result.history.agentDescription) {
      console.log(
        colorize(`   📝 Description: ${colorize(result.history.agentDescription, 'bold')}`, 'cyan')
      );
    }
    ...
    console.log(
      colorize(
        `\n   Run it with: ${colorize(`deno run ${prepResult.files.indexFile}`, 'bold')}`,
        'gray'
      )
    );

    try {
      const viewer = startAgentViewer({
        agentName: name,
        prompt: promptText,
        description: result.history.agentDescription,
        code: result.generation.code,
        executionOutput: result.execution.output,
        location: prepResult.files.indexFile,
        sessionTimestamp: result.history.timestamp,
      });

      console.log(colorize(`\n   🌐 Preview available: ${colorize(viewer.url, 'cyan')}`, 'gray'));
      console.log(
        colorize(
          '      (auto-closes after 2 minutes; leave this command running to keep it open)',
          'gray'
        )
      );
    } catch (viewerError) {
      const err = viewerError as Error;
      console.log(
        colorize(
          `\n   ⚠️  Preview server unavailable: ${colorize(err.message, 'yellow')}`,
          'yellow'
        )
      );
    }
    ...
    if (result.execution.success && result.execution.output) {
      ...
      if (output.length > 0) {
        ...
        const previewOutput =
          output.length > maxPreviewLength
        ...
        console.log(
          colorize('   (Agent executed successfully but produced no visible output)', 'gray')
        );
      }
 ...
 */
async function refineAgentInteractive(name: string, previousResult: any): Promise<void> {
  console.log(colorize('\n💡 Refinement Mode', 'cyan'));
  console.log(
    colorize('   You can now improve the generated code based on your feedback.', 'gray')
  );
...
async function startInteractiveMode(): Promise<void> {
  ...
  while (continueLoop) {
    ...
    if (quickStartChoice === 'fizzbuzz') {
      ...
      console.log(
        colorize('   Demonstrates: Browserbase/Stagehand for AI-powered web browsing', 'gray')
      );
      console.log(colorize(`   Prompt: "${promptText.substring(0, 80)}..."`, 'gray'));
      ...
      console.log(
        colorize('   Demonstrates: Mastra framework with agents, memory, and workflows', 'gray')
      );
      console.log(colorize(`   Prompt: "${promptText.substring(0, 80)}..."`, 'gray'));
    } else {
      // Custom - ask user
    ...
    if (existingLocation) {
      ...
      console.log(
        colorize(
          `\n📍 Continuing existing ${isUtility ? 'utility' : 'agent'}: ${locationLabel}${name}`,
          'cyan'
        )
      );
    } else {
...
async function handleCommandLine(args: string[]): Promise<void> {
  ...
  if (command === 'promote') {
    ...
    if (!name) {
      ...
      console.log(
        colorize('Usage: deno task cli:promote <agent-name> [--dry-run] [--force]', 'gray')
      );
      Deno.exit(1);
    }
...
function displayHelp(): void {
  ...
  console.log(
    '  # → Choose from: FizzBuzz, Joke Generator, Web Search, AI Browser, Multi-Agent, or Custom\n'
  );
  ...
  console.log(
    '  deno task cli:promote jokemeister           # Move to utils/ with AI-generated examples.ts'
  );
