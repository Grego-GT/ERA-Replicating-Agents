...
import { chat, type ChatMessage } from './wandb.ts';
...
- If output contains TypeScript errors (TS), compilation failed → Clear failure
- If output contains runtime exceptions → Clear failure
- If output is null/empty → Likely failure
...
- Examples:
  - Prompt: "reverse hello world" → Output: "dlrow olleh" ✅ Success
  - Prompt: "calculate factorial of 5" → Output: "120" ✅ Success
  - Prompt: "sum 1 to 100" → Output: "5050" ✅ Success
  - Prompt: "sort array" → Output: "[1,2,3,4,5]" ✅ Success
- Does it look like an error message? → Failure
- Is it empty or null? → Failure
...
- ✅ Code executed successfully AND output matches user's intent (JSON or plain text)
- ✅ Output demonstrates correct result (even if format is different than expected)
- ⚠️ No progress in last 2 iterations (stuck in same error)
- ⚠️ Issue is not solvable through prompt refinement (environment/API limitation)
- ⚠️ Errors are becoming worse, not better

### CONTINUE Iterating (shouldRetry: true) if:
- ❌ Execution failed (compilation error, runtime error, or Daytona error)
- ❌ Output is empty, null, or clearly wrong for the user's request
- 📈 Making progress (different errors or getting closer to correct output)
- 🔧 Clear path to fix the issue with better prompt
- 📊 Haven't tried an obvious approach yet
...
async function improvePrompt(
  ...
  try {
    ...
    if (!parsedResponse.improvedPrompt || !Array.isArray(parsedResponse.improvements)) {
      console.warn('⚠️ Invalid Director response structure, using original prompt');
      return {
        success: false,
        originalPrompt: userPrompt,
        improvedPrompt: userPrompt,
    ...
    console.error('❌ FBI Director failed:', err.message);
...
async function makeVerdict(
  ...
  try {
    console.log(`⚖️  FBI Director reviewing ${allAttempts.length} attempt(s) to make verdict...`);
    ...
    try {
      ...
      console.warn('⚠️  Failed to parse Director verdict, defaulting to retry');
      console.warn('Raw response:', content);
    ...
    if (typeof verdict.shouldRetry !== 'boolean') {
      console.warn('⚠️  Invalid verdict structure, defaulting to retry');
      return {
        shouldRetry: currentIteration < maxIterations,
    ...
    const err = error as Error;
    console.error('❌ Director verdict failed:', err.message);
...
export async function testDirector(): Promise<void> {
  console.log('🚀 Testing FBI Director Prompt Improvement...\n');
  ...
  for (let i = 0; i < testPrompts.length; i++) {
    ...
    console.log('\n' + '='.repeat(80));
    console.log(`📋 Test ${i + 1}/${testPrompts.length}: "${test.prompt}" (${test.language})`);
    ...
    try {
      ...
      if (result.success) {
        console.log(`\n📝 Original Prompt (${result.originalPrompt.length} chars):`);
        console.log(result.originalPrompt);
        
        console.log(`\n✨ Improved Prompt (${result.improvedPrompt.length} chars):`);
